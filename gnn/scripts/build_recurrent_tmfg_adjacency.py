...
sys.path.insert(0, str(ROOT / "tmfg"))

from TMFG_core import TMFG, OutputMode  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input",
        default="../csv_NMI_matrix/lob_similarity_nmi_rellag_lag150_bins2000_mean.csv",
        help="NMI/similarity CSV path, relative to gnn/ by default.",
    )
    parser.add_argument(
        "--output",
...
def main() -> None:
    ...
    if not input_path.exists():
        available = sorted((ROOT / "csv_NMI_matrix").glob("*.csv"))
